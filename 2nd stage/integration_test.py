...
# Environment
AON_env = environment.env()
# Agent
AON_env = environment.env()
AON_agent = SAC(AON_env.observation_space.shape[0], AON_env.attack_action_space, args)
agent.load_checkpoint('Zsofarsogood_16_25.111777437874117.pt')
AON_agent.load_checkpoint('sofarsogood_63_success_20_fuel_5.7833.pt')
time_start=time.time()
episodes = 100
done_num=0
avg_fuel=0
...
while done_num<episodes:
    ...
    while not done:
        ...
        if info:
            done_num+=1
            AON_state=AON_env.input_reset(Xt[-1],Vt[-1],Xc[-1],Vc[-1])
            AON_done = False
            steps=0
            AON_fuel_consumption=0
            while not AON_done:
                AON_action = AON_agent.select_action(AON_state, evaluate=True) #evaluate为True时为确定性网络，直接输出mean
                AON_fuel_consumption=AON_fuel_consumption+np.linalg.norm(AON_action[0:3])
                AON_next_state, AON_reward, AON_done, AON_info,_,AON_distance = AON_env.step(AON_action)
                AON_state = AON_next_state
                steps+=1
                if AON_info:
                    AON_done_num+=1
                    AON_avg_fuel+=AON_fuel_consumption
            AON_avg_steps+=steps
            #if info:
...
# avg_steps/= episodes
AON_avg_fuel /= AON_done_num
#writer.add_scalar('avg_reward/test', avg_reward, i_episode)
time_end=time.time()
print("----------------------------------------")
print("Test Episodes: {},done num:{},fuel consumption：{}".format(episodes,AON_done_num,AON_avg_fuel))
print("----------------------------------------")
