...
class env:
    ...
    def reset(self,seed=None): #,prop_t
        ...
        state.append(o1)
        state.append(o2)
        # state.append(self.N_dis(o1))
        # state.append(o2)
        # state.append(self.N_dis(o2))
        o5,o6=self.angle_observe(self.blue_pos,self.blue_vel,self.red_pos,self.red_vel,0)
        state.append(o5)
        state.append(o6)
        # state.append(self.N_dis(o5))
        # state.append(o6)
        # state.append(self.N_dis(o6))
        state.append(0)
        state.append(0)
        state.append(0)
        ...
        state.append(o9)
        state.append(o10)
        # state.append(self.N_dis(o9))
        # state.append(o10)
        # state.append(self.N_dis(o10))
    ...
    def step(self,blue_action):#,red_action
        ...
        o9,o10=self.angle_observe(self.blue_pos,self.blue_vel,self.red_pos,self.red_vel+red_impulse,self.interval)
        state.append(o9)
        state.append(o10)
        # state.append(self.N_dis(o9))
        # state.append(o10)
        # state.append(self.N_dis(o10))
        middle_red_pos,middle_red_vel=CW_Prop(self.red_pos,self.red_vel+red_impulse,self.omega,self.interval) #下一时刻前300s机动，另一种随机给机动是本时刻后300s给机动，这个不行就试另一种
        #if self.i%4==0:
        ...
        if not self.done:
            reward=-self.distance/1e4-np.linalg.norm(blue_action[0:3])/2 #-np.linalg.norm(self.blue_vel-self.target_vel)/30
