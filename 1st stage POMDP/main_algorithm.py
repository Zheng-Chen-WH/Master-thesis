...
class test_env:
    ...
    def step(self,blue_action):#,red_action
        ...
        '''【设定目标逃逸策略】'''
        delay=np.random.uniform(0,3600)
        middle_red_pos,middle_red_vel=CW_Prop(self.red_pos,self.red_vel,self.omega,delay)
        # red_impulse=self.opt_escape(middle_red_pos,middle_red_vel,self.blue_pos,self.blue_vel) #最优跑
        # red_impulse=self.pos_escape(middle_red_pos,middle_red_vel,self.blue_pos,self.blue_vel) #位置反向跑
        ...
        self.red_vel=middle_red_vel+red_impulse
        self.next_red_pos,self.next_red_vel=CW_Prop(middle_red_pos,self.red_vel,self.omega,3600-delay)
