...
if args['task']=='Test':
    agent.load_checkpoint('sofarsogood_8_success_20_fuel_8.7989.pt')
    time_start=time.time()
    avg_reward = 0
    episodes = 1000
    done_num=0
...
if args['task']=='Plot':
    ...
    while not done:
        ...
        Vt.append(red_vel)
        Vc.append(blue_vel)
        target_pos.append(goal[0:3]*1000)
        target_vel.append(goal[3:])
        state = next_state
    print("----------------------------------------")
    ...
    pos_x_objective=np.array([array[0] for array in target_pos])
    pos_y_objective=np.array([array[1] for array in target_pos])
    pos_z_objective=np.array([array[2] for array in target_pos])
    vel_x_objective=np.array([array[0] for array in target_vel])
    vel_y_objective=np.array([array[1] for array in target_vel])
    vel_z_objective=np.array([array[2] for array in target_vel])
