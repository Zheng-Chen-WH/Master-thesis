...
from HERsac import SAC
import replay_memory
import WHOLEsimulation
import time
...
args={'policy':"Gaussian", # Policy Type: Gaussian | Deterministic (default: Gaussian)
        ...
        'memory_type':'HER', #选择memory形式，Normal或HER
        'HER-k':10, #重设目标的时候如果采用后三种方式，各生成几条轨迹
        'HERmethod':'future',} #HER的目标重取方式，final — 直接取同一条轨迹最后状态作为新目标
                               #future — 从该时刻（？）起往后的同一序列中的状态，随机采样k个作为新的目标goal，得到k条新轨迹
                               #episode — 对于同一序列中的状态，随机采样k个作为新的目标goal
                               #random — 从整个经验池里随机抽k个

# Environment
env = WHOLEsimulation.env()
# Agent
...
if args['memory_type']=='Normal':
    memory = replay_memory.ReplayMemory(args['replay_size']) #,args['HERmethod'],args['HER-k'])
elif args['memory_type']=='HER':
    memory = replay_memory.HERMemory(args['replay_size'], args['HERmethod'], args['HER-k'])

if args['task']=='Train':
    if args['logs']==True:
        from torch.utils.tensorboard import SummaryWriter
        writer = SummaryWriter('./runs/HER training using shaping and future/')
    # Training Loop
    updates = 0
    best_avg_reward=-500
    total_numsteps=0
    ...
    if args['LOAD PARA']==True:
        agent.load_checkpoint("sofarsogood_87_success_20_fuel_11.5649.pt")
        best_avg_reward=0
    ...
    for i_episode in itertools.count(1): #itertools.count(1)用于创建一个无限迭代器。它会生成一个连续的整数序列，从1开始，每次递增1。
        ...
        while True:
            ...
            # else:
            action = agent.select_action(np.concatenate((state,goal)))  # 开始输出actor网络动作
            ...
            memory.push(state, action, reward, next_state, done, goal, info) # Append transition to memory
            state = next_state
        ...
        if i_episode % args['evaluate_freq'] == 0 and args['eval'] is True: #评价上一个训练过程
            ...
            for _  in range(episodes):
                ...
                steps=0
                while not done:
                    action = agent.select_action(np.concatenate((state,goal)), evaluate=True) #evaluate为True时为确定性网络，直接输出mean
                    next_state, reward, done, info,fuel,_ = env.step(action)
                    memory.push(state, action, reward, next_state, done,goal,info)
...
if args['task']=='Test':
    agent.load_checkpoint('3rd stage fine-tuned.pt')
    time_start=time.time()
    avg_reward = 0
    episodes = 1
    done_num=0
    ...
    for i  in range(episodes):
        ...
        while not done:
            action = agent.select_action(np.concatenate((state,goal)), evaluate=True) #evaluate为True时为确定性网络，直接输出mean
            next_state, reward, done, info,fuel_used,distance = env.step(action)
            episode_reward += reward
            state = next_state
            steps+=1
            if info:
...
if args['task']=='Plot':
    agent.load_checkpoint('3rd stage fine-tuned.pt')
    state, goal = env.reset(99999)
    print(goal)
    done = False
    while not done:
        action = agent.select_action(np.concatenate((state,goal)), evaluate=True) #evaluate为True时为确定性网络，直接输出mean
        next_state,_, done, info,fuel,_, Xt, Xc, Vt, Vc,time_sequence = env.step(action)
        target_pos=goal[0:3]*1000
        target_vel=goal[3:]
        state = next_state
    print("----------------------------------------")
    print("done:{},fuel:{}".format(info,fuel))
    print("----------------------------------------")
    ...
    pos_x_objective = target_pos[0] * np.ones(len(pos_x_target))
    pos_y_objective=target_pos[1] * np.ones(len(pos_x_target))
    pos_z_objective=target_pos[2] * np.ones(len(pos_x_target))
    vel_x_objective=target_vel[0] *np.ones( len(pos_x_target))
    vel_y_objective=target_vel[1] *np.ones( len(pos_x_target))
    vel_z_objective=target_vel[2] *np.ones( len(pos_x_target))
    ...
    ax2.plot(time_sequence,(pos_x_chaser-pos_x_target)/1e3,'r',linewidth=1, label="x distance") 
    ax2.plot(time_sequence,(pos_y_chaser-pos_y_target)/1e3,'b',linewidth=1, label="y distance") 
    ax2.plot(time_sequence,(pos_z_chaser-pos_z_target)/1e3,'g',linewidth=1, label="z distance")
    ax2.plot(time_sequence,(pos_x_objective)/1e3,'r',linewidth=1, linestyle='--') 
    ax2.plot(time_sequence,(pos_y_objective)/1e3,'b',linewidth=1, linestyle='--') 
    ax2.plot(time_sequence,(pos_z_objective)/1e3,'g',linewidth=1, linestyle='--')
    ax3.plot(time_sequence,velo_x_chaser-velo_x_target,'r',linewidth=1, label="x velocity") 
    ax3.plot(time_sequence,velo_y_chaser-velo_y_target,'b',linewidth=1, label="y velocity") 
    ax3.plot(time_sequence,velo_z_chaser-velo_z_target,'g',linewidth=1, label="z velocity")
    ax3.plot(time_sequence,vel_x_objective,'r',linewidth=1, linestyle='--') 
    ax3.plot(time_sequence,vel_y_objective,'b',linewidth=1, linestyle='--') 
    ax3.plot(time_sequence,vel_z_objective,'g',linewidth=1, linestyle='--')
