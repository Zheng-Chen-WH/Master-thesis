...
class pre_env:
    ...
    def target_generate(self,seed): #生成能让仅测角安心工作的目标位置
        ...
        print(relative_pos,relative_vel)
    ...
    def reset(self,seed=None): #,prop_t
        ...
        self.t_sequence.append(self.time)
    ...
    def step(self,blue_action):#,red_action
        ...
        if self.done:
            if self.fuel[0]<0:
                ...
                self.info=1
                print(30-self.fuel[0])
            self.i+=1

        return state, self.done, self.info, self.Xt, self.Xc, self.Vt, self.Vc, self.t_sequence, 30-self.fuel[0]
...
class AON_env:
    ...
    def reset(self,seed=None): #,prop_t
        ...
        while not info:
            ...
            while not done:
                ...
                next_state, done, info, self.Xt, self.Xc, self.Vt, self.Vc, self.t_sequence, self.fuel_used = self.pre_env.step(action)
                state = next_state
        self.blue_pos_list=[]
        #self.red_pos_list=[]
        self.info=0
        self.done=False
        self.blue_pos=self.Xc[-1]
        self.blue_vel=self.Vc[-1]
        self.red_pos=self.Xt[-1]
        self.red_vel=self.Vt[-1]
        self.time=self.t_sequence[-1]
    ...
    def step(self,blue_action):#,red_action
        ...
        self.time=self.time+blue_action[3]
        self.Xt.append(self.red_pos)
        self.Vt.append(self.red_vel)
        self.Xc.append(self.blue_pos)
        self.Vc.append(self.blue_vel)
        ...
        if self.done:
            if self.fuel[0]<0:
                ...
                self.info=1
                print(20-self.fuel[0])
            self.i+=1

        return state, self.done, self.info, self.Xt, self.Xc, self.Vt, self.Vc, self.t_sequence, self.fuel_used+20-self.fuel[0]
    ...
    def plotstep(self,blue_action):
        ...
        return state, self.done, self.info, self.red_pos, self.blue_pos, self.red_vel, self.blue_vel, self.t_sequence
...
class env:
    ...
    def reset(self,seed=None): #,prop_t
        ...
        while not info:
            ...
            while not done:
                action = self.agent.select_action(state, evaluate=True) #evaluate为True时为确定性网络，直接输出mean
                next_state, done, info, self.Xt, self.Xc, self.Vt, self.Vc, self.t_sequence, self.fuel_used = self.AON_env.step(action)
                state = next_state
        self.info=0
        self.done=False
        self.blue_pos=self.Xc[-1]
    ...
    def step(self,blue_action):#,red_action
        ...
        return state, reward, self.done, self.info, self.fuel_used+20-self.fuel[0], self.distance,self.Xt,self.Xc,self.Vt,self.Vc,self.t_sequence
