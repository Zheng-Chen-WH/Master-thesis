...
class SAC(object):
    def __init__(self, num_inputs, action_space, args):
        ...
        self.critic = QNetwork(num_inputs, action_space.shape[0], args['hidden_sizes'], args['activation']).to(device=self.device)
        self.critic_optim = Adam(self.critic.parameters(), lr=args['lr'])

        self.critic_target = QNetwork(num_inputs, action_space.shape[0], args['hidden_sizes'], args['activation']).to(self.device)
    ...
    def update_parameters(self, memory, batch_size, updates):
        ...
        state_batch, action_batch, reward_batch, next_state_batch, done_batch, goal_batch = memory.sample(batch_size=batch_size)
        # 这里的所有batch都是一整个数组
        ...
        with torch.no_grad():
            next_state_action, next_state_log_pi, _ = self.policy.sample(torch.cat([next_state_batch,goal_batch],dim=1)) #policy网络算出来的action
            qf1_next_target, qf2_next_target = self.critic_target(torch.cat([next_state_batch,goal_batch],dim=1), next_state_action) #target算出来的q值
        ...
        qf1, qf2 = self.critic(torch.cat([state_batch,goal_batch],dim=1), action_batch)  # Two Q-functions to mitigate positive bias in the policy improvement step
        ...
        pi, log_pi, _ = self.policy.sample(torch.cat([state_batch,goal_batch],dim=1))

        qf1_pi, qf2_pi = self.critic(torch.cat([state_batch,goal_batch],dim=1), pi)
